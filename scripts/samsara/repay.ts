#!/usr/bin/env tsx
/**
 * Build an unsigned repay transaction (repay borrowed base token debt).
 *
 * Usage: npx tsx scripts/samsara/repay.ts <amount> --market <name> [--keypair <path>] [--rpc <url>] [--verbose]
 *
 * Arguments:
 *   <amount>           Amount of base token to repay (in human units)
 *
 * Options:
 *   --market <name>    Market to repay in (default: navSOL)
...
async function main() {
  ...
  if (!amountStr) {
    console.error('Usage: npx tsx scripts/samsara/repay.ts <amount> --market <name> [--keypair <path>] [--rpc <url>]');
    console.error('');
  ...
  logVerbose(verbose, `Repay amount: ${amount} ${market.baseName}`);
  ...
  const txBytes = await client.buildUnsignedRepayTransaction({
