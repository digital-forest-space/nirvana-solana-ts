#!/usr/bin/env tsx
/**
 * Build an unsigned withdraw navToken transaction (unstake navToken from market).
 *
 * Usage: npx tsx scripts/samsara/withdraw-nav.ts <amount> --market <name> [--keypair <path>] [--rpc <url>] [--verbose]
 *
 * Arguments:
 *   <amount>           Amount of navToken to withdraw (in human units)
 *
 * Options:
 *   --market <name>    Market to withdraw from (default: navSOL)
...
async function main() {
  ...
  if (!amountStr) {
    console.error('Usage: npx tsx scripts/samsara/withdraw-nav.ts <amount> --market <name> [--keypair <path>] [--rpc <url>]');
    console.error('');
  ...
  const txBytes = await client.buildUnsignedWithdrawNavTokenTransaction({
