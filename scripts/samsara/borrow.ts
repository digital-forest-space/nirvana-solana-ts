#!/usr/bin/env tsx
/**
 * Build an unsigned borrow transaction (borrow base token against navToken position).
 *
 * Usage: npx tsx scripts/samsara/borrow.ts <amount> --market <name> [--keypair <path>] [--rpc <url>] [--verbose]
 *
 * Arguments:
 *   <amount>           Amount of base token to borrow (in human units)
 *
 * Options:
 *   --market <name>    Market to borrow from (default: navSOL)
...
async function main() {
  ...
  if (!amountStr) {
    console.error('Usage: npx tsx scripts/samsara/borrow.ts <amount> --market <name> [--keypair <path>] [--rpc <url>]');
    console.error('');
