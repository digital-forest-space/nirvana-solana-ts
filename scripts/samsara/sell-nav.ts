#!/usr/bin/env tsx
/**
 * Build an unsigned sell navToken transaction (navToken -> base token).
 *
 * Usage: npx tsx scripts/samsara/sell-nav.ts <amount> --market <name> [--keypair <path>] [--rpc <url>] [--verbose]
 *
 * Arguments:
 *   <amount>           Amount of navToken to sell (in human units)
 *
 * Options:
 *   --market <name>    Market to sell in (default: navSOL)
...
async function main() {
  ...
  if (!amountStr) {
    console.error('Usage: npx tsx scripts/samsara/sell-nav.ts <amount> --market <name> [--keypair <path>] [--rpc <url>]');
    console.error('');
  ...
  const inputLamports = toLamports(amount, market.navDecimals);

  logVerbose(verbose, `Input lamports: ${inputLamports}`);
  logVerbose(verbose, 'Building unsigned transaction...');

  const txBytes = await client.buildUnsignedSellNavSolTransaction({
