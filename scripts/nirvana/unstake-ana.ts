#!/usr/bin/env tsx
/**
 * Build an unsigned unstake ANA instruction (withdraw ANA).
 *
 * Usage: npx tsx scripts/nirvana/unstake-ana.ts <ana_amount> [--rpc <url>] [--keypair <path>] [--verbose]
 *
...
async function main() {
  ...
  if (!amountStr || !keypairPath) {
    console.error('Usage: npx tsx scripts/nirvana/unstake-ana.ts <ana_amount> [--rpc <url>] [--keypair <path>] [--verbose]');
    console.error('');
  ...
  logVerbose(verbose, `Unstaking: ${anaAmount} ANA`);
  ...
  const instruction = builder.buildWithdrawAnaInstruction(
    userAddress,
    personalAccount,
    accounts.anaAccount,
    anaLamports,
