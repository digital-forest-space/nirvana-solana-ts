#!/usr/bin/env tsx
/**
 * Build an unsigned repay NIRV instruction (burn NIRV to reduce debt).
 *
 * Usage: npx tsx scripts/nirvana/repay-nirv.ts <nirv_amount> [--rpc <url>] [--keypair <path>] [--verbose]
 *
 * Burns NIRV to repay borrowed NIRV debt.
...
async function main() {
  ...
  if (!amountStr || !keypairPath) {
    console.error('Usage: npx tsx scripts/nirvana/repay-nirv.ts <nirv_amount> [--rpc <url>] [--keypair <path>] [--verbose]');
    console.error('');
    ...
    console.error('');
    console.error('Note: Burns NIRV to repay borrowed NIRV debt.');
    process.exit(1);
  }
  ...
  logVerbose(verbose, `Repaying: ${nirvAmount} NIRV`);
  ...
  const instruction = builder.buildRepayInstruction(
    userAddress,
    personalAccount,
