#!/usr/bin/env tsx
/**
 * Build an unsigned claim revenue share instruction (claim accumulated ANA + NIRV revenue).
 *
 * Usage: npx tsx scripts/nirvana/claim-revenue-share.ts [--rpc <url>] [--keypair <path>] [--verbose]
 *
 * No amount needed -- claims all accumulated revenue share.
...
async function main() {
  ...
  if (!keypairPath) {
    console.error('Usage: npx tsx scripts/nirvana/claim-revenue-share.ts [--rpc <url>] [--keypair <path>] [--verbose]');
    console.error('');
  ...
  if (!accounts.anaAccount || !accounts.nirvAccount) {
    console.error(JSON.stringify({
      success: false,
      error: 'Could not resolve required token accounts',
      accounts: { ana: accounts.anaAccount ?? null, nirv: accounts.nirvAccount ?? null },
    }));
    process.exit(1);
  }
  ...
  const instruction = builder.buildClaimRevenueShareInstruction(
    userAddress,
    personalAccount,
    accounts.anaAccount,
